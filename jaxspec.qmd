...
## Normalizing flows {.smaller}
 ...
 Normalizing flows are invertible transformations beotween probability distributions, parametrized using neural networks.

![](images/flow_transform_demo.png){width=100% fig-align="center"}

## Simulation based inference (SBI) {.smaller}

Simulation based inference uses normalizing flow to learn either $P(\theta | X)$ or $P(X | \theta )$, given a training set of random parameters $\{\theta\}_i$ and simulated observables $\{X\}_i$ using the same parameters.
